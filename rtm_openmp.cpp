...
#pragma omp parallel for
  ...
  elapse += (stop.tv_sec - start.tv_sec) * 1000000 + (stop.tv_usec - start.tv_usec);
...
void fd4t10s_2d(int nx, int nz, const float *vel, float *prev_wave, float *curr_wave) {
	...
	float *u2 = (float *)malloc(sizeof(float) * nx * nz);
...
void forward_propagate_2d(int nx, int nz, float* vel, float* wav, int nt, float dt, int srcx, int srcz)
{
	...
	float* prev_wave = (float *)malloc(sizeof(float) * model_size);
	float* curr_wave = (float *)malloc(sizeof(float) * model_size);
	...
	for(time_step = 0; time_step < nt; time_step ++)
	{
		...
		fd4t10s_2d(nx, nz, vel, prev_wave, curr_wave);
...
int main(int argc, char ** argv){
  ...
  float image_sum = std::accumulate(image, image + model_size, 0.0f);
  printf("image_sum: %f\n", image_sum);

  gettimeofday(&total_stop, NULL);
  double total_elapse = (total_stop.tv_sec - total_start.tv_sec) * 1000000 + (total_stop.tv_usec - total_start.tv_usec);


  printf("time elpase for computing only: %f\n", elapse / 1000000);
  printf("total time elpase: %f\n", total_elapse / 1000000);
